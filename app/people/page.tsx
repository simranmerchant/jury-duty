...
export default function PeoplePage() {
  ...
  useEffect(() => {
    ...
    getAccessToken().then((token) => {
      return fetch("/api/v1/me/suggestions", {
  ...
  async function handleFollow(userId: string) {
    ...
    const token = await getAccessToken();
    const res = await fetch(`/api/v1/users/${encodeURIComponent(userId)}/follow`, {
      method: "POST",
      headers: { Authorization: `Bearer ${token}` },
    });
    if (res.ok) {
    ...
    <div className="min-h-screen" style={{ background: "var(--bg)", color: "var(--text)" }}>
      ...
      <div className="px-5 pt-4 pb-32 flex flex-col gap-4">
        ...
        {showSuggestions && (
          <>
            {suggestionsLoading && (
              <p className="text-center text-[13px] py-4" style={{ color: "var(--dimmer)" }}>loading...</p>
            )}

            {!suggestionsLoading && suggestions.length > 0 && (
              <>
                <p className="text-[12px] font-semibold px-1" style={{ color: "var(--dimmer)" }}>
                        ...
                        >
                          ...
                          <div className="flex flex-col gap-0.5 min-w-0">
                            ...
                            <p className="text-[12px]" style={{ color: "var(--muted)" }}>
                              ...
                              {u.username ? " · " : ""}
                              {u.mutual_event_count === 1
                                ? u.mutual_event_names[0] ?? "1 event in common"
                                : `${u.mutual_event_count} events in common`}
                            </p>
                          </div>
                        </button>
                        <button
            ...
            {!suggestionsLoading && suggestions.length === 0 && (
              <div className="flex flex-col items-center gap-2 py-16">
                <p className="text-[15px] font-bold" style={{ color: "var(--muted)" }}>no suggestions yet</p>
                <p className="text-[13px] text-center" style={{ color: "var(--dimmer)" }}>join events to find people to follow</p>
              </div>
            )}
          </>
        )}
