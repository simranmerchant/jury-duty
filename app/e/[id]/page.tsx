...
function BetCard({
  ...
  async function shareToFeed() {
    ...
    const res = await fetch("/api/v1/posts", {
      method: "POST",
      headers: { Authorization: `Bearer ${token}`, "Content-Type": "application/json" },
      body: JSON.stringify({ bet_id: bet.id, caption: shareCaption.trim() || undefined, photo_url: photoUrl, targeted_user_ids: targeted }),
    });
  ...
  return (
    ...
    {showShareFeed && (
      <div
        className="fixed inset-0 z-50 flex items-end justify-center"
        style={{ background: "rgba(0,0,0,0.6)" }}
        onClick={(e) => { if (e.target === e.currentTarget) { setShowShareFeed(false); setShareCaption(""); setSharePhoto(null); setSharePhotoPreview(null); setCaptionMentionSearch(null); setShareAudience("all"); setSelectedFollowers([]); } }}
      >
        <div className="w-full max-w-lg rounded-t-[20px] p-6 flex flex-col gap-4"
          style={{ background: "var(--card)", border: "1px solid var(--border-soft)", maxHeight: "88vh", overflowY: "auto" }}>
          <div className="flex items-center justify-between">
            <h2 className="font-black text-[18px]" style={{ fontFamily: "var(--font-nunito)", color: "var(--text)" }}>share to feed</h2>
            <button onClick={() => { setShowShareFeed(false); setShareCaption(""); setSharePhoto(null); setSharePhotoPreview(null); setCaptionMentionSearch(null); setShareAudience("all"); setSelectedFollowers([]); }} style={{ color: "var(--muted)" }}>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => { setShareAudience("select"); loadFollowers(); }}
                className="flex-1 py-2 rounded-[10px] text-[13px] font-bold"
                ...
                {!followersLoaded ? (
                  ...
                  return (
                    <button key={f.user_id}
                      onClick={() => setSelectedFollowers((s) => selected ? s.filter((id) => id !== f.user_id) : [...s, f.user_id])}
