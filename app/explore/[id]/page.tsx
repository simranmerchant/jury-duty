    ...
    <div className="min-h-screen pb-24" style={{ background: "var(--bg)" }}>
      ...
      <div className="max-w-lg mx-auto px-4 pt-5 flex flex-col gap-5">
        ...
        {hasBet && (
          <div className="rounded-[12px] p-3 flex items-center justify-between gap-3"
            ...
            {/* Share only available after resolution */}
            {isResolved && (
              bet.my_post ? (
                <button
                  onClick={deletePost}
      ...
      {showDelete && (
        <div
          className="fixed inset-0 z-50 flex items-end justify-center"
          style={{ background: "rgba(0,0,0,0.6)" }}
          onClick={(e) => { if (e.target === e.currentTarget) setShowDelete(false); setDeleteError(null); }}
        >
          <div className="w-full max-w-lg rounded-t-[20px] p-6 flex flex-col gap-4"
            style={{ background: "var(--card)", border: "1px solid var(--border-soft)" }}>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setShowDelete(false); setDeleteError(null); }} style={{ color: "var(--muted)" }}>
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
                  <path d="M18 6 6 18M6 6l12 12" />
                </svg>
              </button>
            ...
            {deleteError && <p className="text-[13px]" style={{ color: "var(--loss)" }}>{deleteError}</p>}
            <button
              onClick={deleteBet}
              disabled={deleting}
              className="w-full py-3 rounded-[12px] font-bold text-[15px] text-white transition-opacity"
