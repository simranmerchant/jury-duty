...
import { validatePostUpdate } from "@/lib/share";
...
// PATCH /api/v1/explore-bets/[id]/post — edit caption/photo on an existing post
export async function PATCH(
  ...
  const err = validatePostUpdate({ caption, photo_url });
  if (err) return NextResponse.json({ error: err }, { status: 400 });

  const { error } = await supabase
    .from("explore_bet_posts")
    .update({ caption: typeof caption === "string" ? caption.trim() || null : null, photo_url: photo_url ?? null })
