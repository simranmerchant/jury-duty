...
export async function POST(req: NextRequest) {
  ...
  const { reported_bet_id, reported_user_id, reason } = await req.json();

  if (!reason || !VALID_REASONS.includes(reason)) {
    return NextResponse.json({ error: "invalid reason" }, { status: 400 });
  }
  if (!reported_bet_id && !reported_user_id) {
    return NextResponse.json({ error: "must report a bet or user" }, { status: 400 });
  }
