-- Migration 059: public accounts' posts visible to everyone in feed (temporary)
-- Private accounts' posts remain followers-only.
...
BEGIN
  ...
  FROM (
    ...
    FROM posts p
    WHERE (
      -- standard: poster is followed by viewer, or is the viewer
      p.user_id = ANY(v_feed_user_ids)
      OR (
        -- public accounts' posts are visible to all (temporary)
