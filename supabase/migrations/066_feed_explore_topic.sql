-- Migration 066: add topic info to explore_bet_posts in get_feed.
-- Re-creates the full function from 065 with one addition: a 'topic' subobject
-- inside 'explore_bets' so the mobile feed card can display the topic name.
        ...
        'explore_bets', (
          SELECT jsonb_build_object(
            ...
            'topic', CASE WHEN eb.topic_id IS NOT NULL THEN (
              SELECT jsonb_build_object('id', t.id, 'name', t.name, 'emoji', t.emoji)
