-- Fix: only suppress the raw bet card when the BET CREATOR posts their own bet.
-- Previously any feed user's post would suppress the original, so sharing a
-- friend's bet caused their bet card to disappear.
