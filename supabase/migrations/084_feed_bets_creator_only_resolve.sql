-- Migration 084: feed bets (no event_id) can only be resolved by their creator.
-- Event bets retain existing logic: creator anytime, members/stakers after deadline.
...
begin
  ...
  if p_resolver_id != v_creator and p_resolver_id != v_admin_id then
    -- Feed bets (no event) can only be resolved by their creator.
    if v_event_id is null then
      raise exception 'not authorized to resolve this bet';
    end if;

    -- Event bets: non-creators can only resolve once the deadline has passed.
