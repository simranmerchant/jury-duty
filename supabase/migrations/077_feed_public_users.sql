-- Migration 077: restore public-user post visibility in the feed.
-- Migration 076 narrowed posts/poll_posts/explore_bet_posts to only followed users,
-- which removed the behaviour where posts from non-private (public) accounts are
-- visible to everyone on the app. This migration restores it.
...
BEGIN
  ...
  FROM (
    ...
    WHERE (
      ...
      -- Only suppress raw bet card when the CREATOR posts their own bet.
      -- A non-creator sharing the bet should not hide the original card.
      AND NOT EXISTS (
        SELECT 1 FROM posts p
        WHERE  p.bet_id = b.id
          AND  p.user_id = b.creator_id
