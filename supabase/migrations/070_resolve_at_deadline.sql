-- Migration 070: creator can resolve anytime; group members / stakers can resolve
-- immediately once the deadline passes (remove the 24h grace period).
-- Also allows bet stakers to resolve feed bets after deadline (not just event members).
...
begin
  ...
  if p_resolver_id != v_creator and p_resolver_id != v_admin_id then
    -- Non-creators can only resolve once the deadline has passed.
    if now() < v_deadline then
      raise exception 'not authorized to resolve this bet';
    end if;

    -- Must be an event member (host/guest) OR a staker on this bet.
    select exists(
      ...
      union all
      select 1 from bet_entries where bet_id = p_bet_id and user_id = p_resolver_id
    ) into v_is_member;
