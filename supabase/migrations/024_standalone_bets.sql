-- Allow bets to exist without an event (standalone/shareable bets)
alter table bets alter column event_id drop not null;

-- Invite token for standalone bets (used to share via link)
alter table bets add column invite_token text unique;

-- Update resolve_bet to allow creator-only resolution when event_id is null
...
begin
  ...
  -- For event bets: anyone in the event can resolve 24h after deadline.
  -- For standalone bets (event_id IS NULL): only creator can resolve.
