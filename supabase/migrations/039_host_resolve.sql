-- Allow the event/group host to resolve any bet immediately (not just after 24h).
-- Creators can still always resolve. Everyone else still requires 24h + membership.
...
begin
  ...
  if p_resolver_id != v_creator then
    -- Event host can resolve any bet immediately
    select exists(
      select 1 from events where id = v_event_id and host_id = p_resolver_id
    ) into v_is_host;

    if not v_is_host then
      -- Non-creator, non-host: must wait 24h after deadline
