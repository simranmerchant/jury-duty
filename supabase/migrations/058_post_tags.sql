-- Add post_tags table for photo tagging and expand feed post visibility

CREATE TABLE IF NOT EXISTS post_tags (
  id             uuid DEFAULT gen_random_uuid() PRIMARY KEY,
  post_id        uuid NOT NULL REFERENCES posts(id) ON DELETE CASCADE,
  tagged_user_id text NOT NULL REFERENCES balances(user_id) ON DELETE CASCADE,
  created_at     timestamptz DEFAULT now(),
  UNIQUE(post_id, tagged_user_id)
);

CREATE INDEX IF NOT EXISTS post_tags_post_id_idx        ON post_tags(post_id);
CREATE INDEX IF NOT EXISTS post_tags_tagged_user_id_idx ON post_tags(tagged_user_id);

-- Recreate get_feed: include tagged_users on posts + expand visibility when poster is public
...
BEGIN
  ...
  FROM (
    ...
    FROM posts p
    WHERE (
      -- standard: poster is someone the viewer follows (or is the viewer)
      p.user_id = ANY(v_feed_user_ids)
      OR (
        -- expanded: poster is public AND tagged someone the viewer follows
        NOT EXISTS (SELECT 1 FROM balances WHERE user_id = p.user_id AND is_private = true)
        AND EXISTS (
          SELECT 1 FROM post_tags pt
          WHERE pt.post_id = p.id
            AND pt.tagged_user_id = ANY(v_followed_ids)
        )
      )
    )
      AND (p.targeted_user_ids IS NULL OR p_user_id = ANY(p.targeted_user_ids))
