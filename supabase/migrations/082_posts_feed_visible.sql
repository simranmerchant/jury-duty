-- Migration 082: add feed_visible flag to posts for group-only posts.
-- feed_visible = false means the post appears inside the event/group but NOT in followers' feeds.

ALTER TABLE posts ADD COLUMN IF NOT EXISTS feed_visible boolean NOT NULL DEFAULT true;

-- Recreate get_feed to:
--   1. Filter posts section to only feed_visible = true
--   2. Allow bet dedup subquery to ignore group-only posts (so the raw bet still shows in feed)
...
BEGIN
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'like_count',   (SELECT COUNT(*) FROM bet_likes WHERE bet_id = b.id),
      'liked_by_me',  EXISTS(SELECT 1 FROM bet_likes WHERE bet_id = b.id AND user_id = p_user_id),
  ...
  FROM (
    SELECT jsonb_build_object(
      ...
      'tagged_users', COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'display_name', bl2.display_name,
          'username',     bl2.username,
          'avatar_url',   bl2.avatar_url
        ))
        FROM post_tags pt
        JOIN balances bl2 ON bl2.user_id = pt.tagged_user_id
    ...
    FROM posts p
    WHERE (
      p.user_id = ANY(v_feed_user_ids)
      OR NOT EXISTS (SELECT 1 FROM balances WHERE user_id = p.user_id AND is_private = true)
    )
      AND (p.targeted_user_ids IS NULL OR p_user_id = ANY(p.targeted_user_ids))
      AND p.feed_visible = true
  ...
  IF p_supports_poll_post THEN
    ...
    FROM (
      ...
      WHERE (
        pp.user_id = ANY(v_feed_user_ids)
        OR NOT EXISTS (SELECT 1 FROM balances WHERE user_id = pp.user_id AND is_private = true)
      )
  ...
  IF p_supports_explore_bet THEN
    ...
    FROM (
      SELECT jsonb_build_object(
        ...
        'explore_bets', (
          SELECT jsonb_build_object(
            ...
            'topic', CASE WHEN eb.topic_id IS NOT NULL THEN (
              SELECT jsonb_build_object('id', t.id, 'name', t.name)
              FROM topics t WHERE t.id = eb.topic_id
            ) ELSE NULL END
          )
      ...
      WHERE (
        ebp.user_id = ANY(v_feed_user_ids)
        OR NOT EXISTS (SELECT 1 FROM balances WHERE user_id = ebp.user_id AND is_private = true)
      )
